...
function Dashboard({ patron, onLogout }: { patron: PatronData; onLogout: () => void }) {
  const [tab, setTab] = useState<'overview' | 'bookings' | 'activity' | 'experiences'>('overview')
  ...
  function handleTabChange(t: 'overview' | 'bookings' | 'activity' | 'experiences') {
    setTab(t)
    ...
    <div className="min-h-screen flex flex-col" style={{ background: '#080c08' }}>
      ...
      <div className="px-5 mb-5">
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)' }}>
          {([['overview', 'Overview'], ['bookings', 'Bookings'], ['experiences', 'Explore'], ['activity', 'Activity']] as const).map(([t, label]) => (
            <button key={t} onClick={() => handleTabChange(t)}
              className="flex-1 py-2.5 rounded-lg font-body font-semibold text-xs transition-all"
              style={{
      ...
      <div className="px-5 flex-1 pb-10">
        ...
        {tab === 'experiences' && (
          <div className="space-y-3">
            <p className="text-white/35 text-[10px] font-body uppercase tracking-[0.2em] mb-4">Book an Experience</p>
            {[
              {
              ...
              <button key={exp.service + exp.title} onClick={() => setBookService(exp.service)}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white font-body font-semibold text-sm">{exp.title}</p>
                  <p className="text-white/40 font-body text-xs mt-0.5 leading-relaxed">{exp.desc}</p>
                </div>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="9 18 15 12 9 6"/>
                </svg>
            ...
            <div className="rounded-2xl p-4 mt-2" style={{ background: 'rgba(200,135,58,0.06)', border: '1px solid rgba(200,135,58,0.15)' }}>
              <p className="text-white/60 font-body text-xs leading-relaxed text-center">
                Give your phone number <span className="text-white/80 font-semibold">{patron.phone}</span> at any service point to earn coupons on your visit.
              </p>
            </div>
          </div>
        )}
