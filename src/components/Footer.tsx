...
export default function Footer() {
  ...
  return (
    ...
    <footer style={{ backgroundColor: '#2C1A08' }} className="rounded-3xl overflow-hidden">

      {/* Main grid */}
      <div className="max-w-7xl mx-auto px-6 lg:px-10 pt-14 pb-10">
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-white/10">

          {/* Brand col — spans 2 on mobile, 1 on lg */}
          <div className="col-span-2 lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              <img src="/assets/logo-agencies-noBackground.png" alt="CabHouse" className="h-20 w-auto" />
              <div>
            ...
            <p className="text-white/50 font-body text-xs leading-relaxed mb-5">
            ...
            <div className="flex gap-2">
              <a href={`https://wa.me/${wa}`} target="_blank" rel="noopener noreferrer"
                className="w-8 h-8 rounded-full border border-white/15 hover:border-[#25D366] flex items-center justify-center text-white/40 hover:text-[#25D366] transition-colors">
              ...
              <a href={`mailto:${SITE.contact.email}`}
                className="w-8 h-8 rounded-full border border-white/15 hover:border-brand-gold flex items-center justify-center text-white/40 hover:text-brand-gold transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {COL_BRANDS.map(l => (
                <li key={l.label}>
                  <a href={l.href} className="text-white/60 hover:text-brand-gold font-body text-sm transition-colors">{l.label}</a>
                </li>
              ))}
            </ul>
          </div>
              ...
              {COL_COMPANY.map(l => (
                <li key={l.label}>
                  <a href={l.href} target={l.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                    className="text-white/60 hover:text-brand-gold font-body text-sm transition-colors">{l.label}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="pt-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <p className="text-white/30 font-body text-xs flex items-center gap-1.5">
      ...
      {/* Contact bar */}
      <div className="border-t border-white/8" style={{ backgroundColor: 'rgba(0,0,0,0.20)' }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-white/50 font-body text-sm">
            ...
            <span className="text-white/30">Mon–Sat, 8am–6pm</span>
          </p>
          <div className="flex items-center gap-3">
            ...
            <a href={`tel:${SITE.contact.phone}`} className="text-white/50 hover:text-brand-gold font-body text-sm transition-colors">
              {SITE.contact.phone}
            </a>
          </div>
        </div>
